...
class FirmwareUploader:
    ...
    def check_firmware_files(self) -> bool:
        ...
        if missing_required:
            print(f"[ERROR] Missing required firmware files: {', '.join(missing_required)}")
            return False
    ...
    def upload_firmware(self, port: str) -> bool:
        ...
        if bootloader_bin.exists() and partition_bin.exists():
            ...
            cmd.extend([
                hex(self.FLASH_ADDRESSES["bootloader"]), str(bootloader_bin),
                hex(self.FLASH_ADDRESSES["partition-table"]), str(partition_bin),
                hex(self.FLASH_ADDRESSES["firmware"]), str(firmware_bin),
            ])
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload StripAlerts ESP32 firmware"
    )
    parser.add_argument(
        "--port", "-p",
